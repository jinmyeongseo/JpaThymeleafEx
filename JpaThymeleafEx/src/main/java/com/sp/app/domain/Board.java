...
public class Board {
	...
	// 주의 : 필드명이 ipAddr 처럼 중간에 대문자가 있으면 컬럼명은 ip_addr로 설정되므로 name 속성으로 컬럼명을 모두 소문자로 명시해야 함
	@Column(name = "ipaddr", nullable = false, length = 50, updatable = false)
	private String ipAddr;
	
	@Column(nullable = false, columnDefinition = "DATA DEFAULT SYSDATE", insertable = false, updatable = false)
