...
public class BoardServiceImpl implements BoardService {
	...
	/*
	 - 클래스 레벨에서 @Transactional(readOnly = true) 를 설정해서 모든 메소드는 변경이 불가능하다.
	 - 따라서 insert, update, delete 에서는 @Transactional 애노테이션을 사용하여 변경이 가능하도록 설정해야 함
	 - 클래스 레벨에서 @Transactional(readOnly = true) 를 설정 하지 않으면 메소드 레벨에서 @Transactional 애노테이션을 사용하지 않아도 가능
	 */
	@Transactional
	@Override
